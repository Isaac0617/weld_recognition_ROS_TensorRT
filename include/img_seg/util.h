...
{
public:
    ImageBase(const nvinfer1::Dims& dims);
    virtual ~ImageBase() {}
...
{
public:
    ArgmaxImageWriter(const nvinfer1::Dims& dims, const std::vector<int>& palette, const int num_classes);
    void process(const float* buffer);
private:
    int mNumClasses;
    std::vector<int> mPalette;
    std::vector<int> _colors{0, 255};
};
